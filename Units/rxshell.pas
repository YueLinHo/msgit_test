...
uses RxConst, RxResConst, RxVCLUtils, RxMaxMin;
...
begin
  ...
  begin
    ...
    else hIcon := {$IFNDEF VER80}INVALID_HANDLE_VALUE{$ELSE}0{$ENDIF};
...
begin
  ...
  begin
    if ((csDesigning in ComponentState) and FShowDesign) or
      (not (csDesigning in ComponentState) and FActive)
    then
      Activate;
  end;
end;
...
begin
  ...
  begin
    ...
    if GetIconLocation(PChar(FIconFile), length(FIconFile), FIconIndex) <> NOERROR
    then
      Exit;
...
begin
  ...
  begin
    ...
    if SetWorkingDirectory(PChar(ExtractQuotedString(FWorkDir, '"'))) <> NOERROR
    then
      Exit;
    // set show command
    case FShowCmd of
